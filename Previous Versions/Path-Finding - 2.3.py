...
def horizontalWalls(x1,x2,y):
    for i in range(x1,x2+1,20):
        walls.append(Node(None,i,y))

def verticalWalls(y1,y2,x):
    for i in range(y1,y2+1,20):
        walls.append(Node(None,x,i))
...
def divide(x,y,width,height):
    ...
    if width <= 80:
        ...
        if orientation == 'Horizontal':
            #print(height)
            randy = random.randrange(y,y+height,20)
            horizontalWalls(x,x+width,randy)
            #removeWall(random.randrange(x,x+width+1,20),randy)
            removeWall((x+width)//2,randy//2)
            divide(x,y,width,randy-y)
            divide(x,randy,width,y+height-randy)
        elif orientation == 'Vertical':
            randx = random.randrange(x,x+width,20)
            verticalWalls(y,y+height,randx)
            removeWall(randx//2,(y+height)//2)
            #removeWall(randx,random.randrange(y,y+width+1,20))
            divide(x,y,height,randx-x)
            divide(randx,y,x+width-randx,height)
...
algo = ''
run = True
#divide(0,40,1000,560)
while run:
    win.fill((160,160,160))
    grid()
    coltime += 1
    if coltime%3 == 0 and initiate:
        if col[1] != 255:
            col[1] += 1
    ...
    if placedWall == True:
        ...
        if initiate == False and down == True and closed == []:
            ...
            if foundDest:
                text("Destination Has Been Found",120,2)
            else:
                text("Destination Was Not Found",120,2)
        ...
        if opened != []:
            ...
            if foundDest == False:   
                ...
                for node in successors:
                    if 0<=node.x<1000 and 40<=node.y<600 and nodeIn(node,walls) == False and surroundWall(node) == False:
